...
class Alg(object):
    fpr = 44100                    
    Np = 10                       # liczba biegunow w filtrze predykcji
    Nc = 12                        # liczba wyznaczanych wspolczynnikow cepstralnych
    ...
    def cisza(self, sygnal):
        ...
        nr1_i = 0
        for nr1 in range(int(Nramek)):
            bx = sygnaln[int(nr1*Mstep): int(Mlen + nr1*Mstep)]
            if bx.dot(bx) > prog:
                break
            nr1_i += 1

        nr2_i = Nramek-1
        for nr2 in range(int(Nramek)-1, -1, -1):
            bx = sygnaln[int(nr2*Mstep): int(Mlen + nr2*Mstep)]
            if bx.dot(bx) > prog:
                break
            nr2_i -= 1

        y = sygnal[int(nr1_i*Mstep): int(Mlen + nr2_i*Mstep)]
        return y
    ...
    def dtw(self, Cx, Cwzr, Nwzr):
        ...
        for numer in range(len(Nwzr)):        # porównaj Cx z Cwzr wszystkich wzorców
            ...
            glob[numer] = g[int(Ns)-1, int(Nw)-1] / math.sqrt(int(Ns)**2 + int(Nw)**2)  # % wartość zakumulowana "najkrótszej" ścieżki
...
#     y = sio.loadmat('../glosowe/SG/' + parts[0] + '{}'.format((k+1)))
    y = sio.loadmat('../glosowe/SG/' + file_name)
